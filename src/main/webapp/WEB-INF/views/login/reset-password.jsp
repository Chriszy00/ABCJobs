...
<jsp:include page="../../header.jsp">
	...
	<div
		class="container min-vh-50 card border border-2 border border-dark mx-auto m-5"
		...
		<form:form action="reset" method="post">
			<div class="card-body m-4 pe-5 ps-5">
				...
				<h5 class="card-subtitle mb-4 ">Please create a new password
					for your account</h5>

				<div class="lh-base" style="text-align: justify;">
					<p class="card-text">
						<b>Tips:</b> We strongly recommend using a password that is unique
						to ABC Jobs. Avoid overly simple passwords like 12345, or
						password123. Use a combination of characters, letters and numbers
					</p>
				</div>
			</div>
			<div class="w-50 d-grid gap-2 col-3 mx-auto">
				<div class="mb-3">
					<label for="password" class="form-label">New Password</label> <input
						type="password"
						class="form-control border border-1 border border-dark"
			...
			<button type="submit"
				class="btn btn-custom-button d-grid gap-2 col-3 mx-auto mb-5"
				data-bs-toggle="modal" data-bs-target="#ver" style="width: 30%;">Update
				Password</button>
		</form:form>
	</div>
	...
	<script>
		...
		$(document).ready(function() {
			$('#passwordConfirmation').on('keyup', function() {
				console.log("Event fired"); // Add this line
				var password = $('#password').val();
				var passwordConfirmation = $(this).val();

				if (password === passwordConfirmation) {
					$(this).removeClass('is-invalid');
					$(this).siblings('.invalid-feedback').empty(); // Clear error message
				} else {
